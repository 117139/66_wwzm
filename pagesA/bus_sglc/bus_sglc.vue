			...
			<view class="pz2_box">
				...
				<block v-if="datas.finish">
					...
					<view class="jd_msgbox jd_msgbox2">
						<view v-if="datas.finish.comments!=undefined" class="sg_bz">{{datas.finish.comments?datas.finish.comments:''}}</view>
						<view class="sg_bz_time">{{datas.finish.created_at}}</view>
						<view class="jd_msgbox_img">
							<view class="pz_img" v-for="(item1,inde1x) in getimgarr(datas.finish.photo)">
								<image :src="item1" mode="aspectFill" @tap="pveimg" :data-src="item1" lazy-load="true"></image>
							</view>
